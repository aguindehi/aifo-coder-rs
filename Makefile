	...
	for t in $$TARGETS; do \
	  ...
	  if [ -n "$$CROSS_BIN" ] && printf "%s" "$$t" | grep -Eq -- '-linux-gnu|-linux-musl'; then \
	    ...
	    "$$RUNTIME" run $$PLATFORM_ARG --rm -v "$$(pwd)":/project -w /project "$$IMAGE" \
	      bash -lc "set -e; export HOME=\$${HOME:-/root}; export CARGO_HOME=\$${CARGO_HOME:-\$${HOME}/.cargo}; export RUSTUP_HOME=\$${RUSTUP_HOME:-\$${HOME}/.rustup}; export PATH=\$${CARGO_HOME}/bin:/usr/local/cargo/bin:/usr/local/bin:/usr/bin:/bin:\$${PATH}; if ! command -v cargo >/dev/null 2>&1; then if command -v rustup >/dev/null 2>&1; then rustup set profile minimal || true; rustup toolchain install stable -c rust-src; rustup default stable; else curl -sSf https://sh.rustup.rs | sh -s -- -y --profile minimal --default-toolchain stable; . \"\$${HOME}/.cargo/env\"; fi; fi; if command -v rustup >/dev/null 2>&1; then rustup target add '$$t' || true; fi; cargo --version; cargo build --release --target '$$t'" || echo "Warning: build failed for $$t"; \
