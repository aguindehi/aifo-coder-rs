...
release:
	...
	for t in $$TARGETS; do \
	  ...
	  if [ -n "$$CROSS_BIN" ] && printf "%s" "$$t" | grep -Eq -- '-linux-gnu|-linux-musl'; then \
	    ...
	    "$$RUNTIME" run $$PLATFORM_ARG --rm -e TARGET="$$t" -v "$$(pwd)":/project -w /project "$$IMAGE" \
	      bash -lc "set -e; export DEBIAN_FRONTEND=noninteractive; export HOME=\$${HOME:-/root}; export CARGO_HOME=\$${CARGO_HOME:-\$${HOME}/.cargo}; export RUSTUP_HOME=\$${RUSTUP_HOME:-\$${HOME}/.rustup}; export PATH=\$${CARGO_HOME}/bin:/usr/local/cargo/bin:/usr/local/bin:/usr/bin:/bin:\$${PATH}; if command -v apt-get >/dev/null 2>&1; then apt-get update && apt-get install -y --no-install-recommends curl ca-certificates clang lld pkg-config && update-ca-certificates || true; elif command -v dnf >/dev/null 2>&1; then dnf install -y curl ca-certificates clang lld pkgconfig || true; elif command -v apk >/dev/null 2>&1; then apk add --no-cache curl ca-certificates clang lld pkgconfig || true; fi; if ! command -v cargo >/dev/null 2>&1; then if command -v rustup >/dev/null 2>&1; then rustup set profile minimal || true; rustup toolchain install stable -c rust-src; rustup default stable; else curl -sSf https://sh.rustup.rs | sh -s -- -y --profile minimal --default-toolchain stable; . \"\$${HOME}/.cargo/env\"; fi; fi; if command -v rustup >/dev/null 2>&1; then rustup target add \"\$${TARGET}\" || true; fi; export CC=clang CXX=clang++ AR=llvm-ar RANLIB=llvm-ranlib PKG_CONFIG_ALLOW_CROSS=1; export CARGO_BUILD_JOBS=\$${CARGO_BUILD_JOBS:-1}; export CARGO_TARGET_X86_64_UNKNOWN_LINUX_GNU_LINKER=clang; export RUSTFLAGS=\"-C link-arg=-fuse-ld=lld \$${RUSTFLAGS}\"; CARGOBIN=\"\$$(command -v cargo || echo \"\$${CARGO_HOME}/bin/cargo\" || echo \"/root/.cargo/bin/cargo\")\"; \"\$${CARGOBIN}\" --version; \"\$${CARGOBIN}\" build --release --target \"\$${TARGET}\"" || echo "Warning: build failed for $$t"; \
