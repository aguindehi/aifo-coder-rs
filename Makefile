...
ifeq ($(OS),Windows_NT)
  ...
  CARGO_VERSION_CMD := $(POWERSHELL) -NoProfile -Command \
    '(Get-Content "Cargo.toml") | ForEach-Object { if($$_ -match "^\s*version\s*=\s*`"(.*)`""){ $$matches[1] } } | Select-Object -First 1'
  CARGO_NAME_CMD := $(POWERSHELL) -NoProfile -Command \
    '(Get-Content "Cargo.toml") | ForEach-Object { if($$_ -match "^\s*name\s*=\s*`"(.*)`""){ $$matches[1] } } | Select-Object -First 1'
else
