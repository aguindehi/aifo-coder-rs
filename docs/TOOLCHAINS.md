# Toolchains Guide (Phase 2–4)

This document explains how to use language toolchains with aifo-coder, including sidecar containers, PATH shims, the toolexec proxy, caches, and the optional Linux unix-socket transport.
...
- Under the hood:
  ...
  - Shim resolves AIFO_TOOLEEXEC_URL=unix:///run/aifo/toolexec.sock and connects via UnixStream.
...
- Caches are enabled by default and persist across runs via named Docker volumes:
  ...
  - C/C++: aifo-ccache
  - Go: aifo-go
- Disable caches for a single run:
  - --no-toolchain-cache
- Purge caches:
