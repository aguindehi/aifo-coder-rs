...
#[test]
fn int_test_cli_doctor_reports_editors_when_image_present() {
    if !docker_present() {
        eprintln!("skipping: docker not found in PATH");
        return;
    }
    // Require at least one of the crush images locally
    ...
    assert!(
        err.contains("editors:") || err.contains("editors ("),
        "doctor output should include an editors line; stderr:\n{}",
        err
    );
}
