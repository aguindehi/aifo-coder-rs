...
// Unix-only capture is sufficient for our CI matrix (macOS/Linux); Windows skips color TTY anyway.
fn capture_stdout<F: FnOnce()>(f: F) -> String {
    use libc::{dup, dup2, fflush, fileno, fopen, STDOUT_FILENO};
    unsafe {
        // Open a temporary file
        let path = std::ffi::CString::new("/tmp/aifo-coder-test-stdout-plain-color.tmp").unwrap();
