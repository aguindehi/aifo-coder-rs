...
fn test_fork_clean_protects_submodule_dirty() {
    ...
    assert!(Command::new("git").args(["init"]).current_dir(&upstream).status().unwrap().success());
    let _ = Command::new("git").args(["config", "user.name", "UT"]).current_dir(&upstream).status();
    let _ = Command::new("git").args(["config", "user.email", "ut@example.com"]).current_dir(&upstream).status();
    std::fs::write(upstream.join("a.txt"), "a\n").unwrap();
    assert!(Command::new("git").args(["add", "-A"]).current_dir(&upstream).status().unwrap().success());
    assert!(Command::new("git").args(["commit", "-m", "sm init"]).current_dir(&upstream).status().unwrap().success());
    ...
    assert!(Command::new("git").args(["commit", "-m", "root"]).current_dir(&pane).status().unwrap().success());
    ...
    assert!(Command::new("git")
        ...
        .unwrap()
        .success());
    assert!(Command::new("git").args(["commit", "-m", "add submodule"]).current_dir(&pane).status().unwrap().success());
    ...
    let _ = Command::new("git").args(["config", "user.name", "UT"]).current_dir(&subdir).status();
    let _ = Command::new("git").args(["config", "user.email", "ut@example.com"]).current_dir(&subdir).status();
    std::fs::write(subdir.join("b.txt"), "b\n").unwrap();
    assert!(Command::new("git").args(["add", "-A"]).current_dir(&subdir).status().unwrap().success());
    assert!(Command::new("git").args(["commit", "-m", "advance sub"]).current_dir(&subdir).status().unwrap().success());
