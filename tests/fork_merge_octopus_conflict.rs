...
fn test_fork_merge_octopus_conflict_sets_meta_and_leaves_branches() {
    ...
    // Pane 1 writes conflict.txt
    {
        ...
        assert!(Command::new("git").args(["commit", "-m", "pane1"]).current_dir(pane_dir).status().unwrap().success());
    }
    // Pane 2 writes conflicting content
    {
        ...
        let _ = Command::new("git").args(["config", "user.email", "aifo@example.com"]).current_dir(pane_dir).status();
        assert!(Command::new("git").args(["add", "-A"]).current_dir(pane_dir).status().unwrap().success());
        assert!(Command::new("git").args(["commit", "-m", "pane2"]).current_dir(pane_dir).status().unwrap().success());
    }
    ...
    let out2 = Command::new("git").args(["ls-files", "-u"]).current_dir(repo).output().unwrap();
