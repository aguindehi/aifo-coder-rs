...
fn connect(url: &str) -> (TcpStream, String, u16, String) {
    ...
    let connect_host = if host == "host.docker.internal" { "127.0.0.1".to_string() } else { host.to_string() };
...
fn test_proxy_handles_large_payload_notifications_cmd() {
    ...
    let mut body = format!("tool={}&cwd={}", urlencoding::Encoded::new("notifications-cmd"), urlencoding::Encoded::new("."));
    for i in 0..5000 {
        body.push('&');
        body.push_str(&format!("arg={}", urlencoding::Encoded::new(&format!("x{i:04}"))));
    }
