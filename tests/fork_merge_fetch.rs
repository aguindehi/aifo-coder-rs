...
fn test_fork_merge_fetch_creates_branches_and_meta() {
    ...
    for (idx, (pane_dir, _br)) in clones.iter().enumerate() {
        ...
        assert!(Command::new("git").args(["commit", "-m", &format!("pane {}", idx + 1)]).current_dir(pane_dir).status().unwrap().success());
    }

    let res = aifo_coder::fork_merge_branches_by_session(repo, sid, aifo_coder::MergingStrategy::Fetch, true, false);
    ...
    assert!(meta.contains("\"merge_strategy\"") && meta.contains("fetch"), "meta should include merge_strategy=fetch, got: {}", meta);
}
