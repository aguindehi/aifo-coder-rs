#[test]
fn unit_test_preview_mounts_for_opencode_include_config_share_cache() {
...
#[test]
fn unit_test_preview_mounts_for_openhands_include_user_dir() {
    std::env::remove_var("AIFO_CODER_FORK_STATE_DIR");
    let td = tempfile::tempdir().expect("tmpdir");
    std::env::set_var("HOME", td.path());
...
#[test]
fn unit_test_preview_mounts_for_plandex_include_home_dir() {
    std::env::remove_var("AIFO_CODER_FORK_STATE_DIR");
    let td = tempfile::tempdir().expect("tmpdir");
    std::env::set_var("HOME", td.path());
