...
fn test_fork_clone_and_checkout_panes_lfs_marker_does_not_fail_without_lfs() {
    ...
    assert!(Command::new("git").args(["commit", "-m", "add lfs marker"]).current_dir(repo).status().unwrap().success());
    ...
    let res = aifo_coder::fork_clone_and_checkout_panes(repo, "sid-lfs", 1, &cur_branch, &base_label, false)
