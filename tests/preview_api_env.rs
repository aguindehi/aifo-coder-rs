...
fn test_build_docker_cmd_includes_api_env_mappings() {
    ...
    assert!(preview.contains("-e OPENAI_API_KEY=k-123"), "missing OPENAI_API_KEY mapping: {preview}");
    ...
    assert!(preview.contains("-e AZURE_API_KEY=k-123"), "missing AZURE_API_KEY mapping: {preview}");
    assert!(preview.contains("-e AZURE_OPENAI_ENDPOINT=https://example.invalid/base"), "missing AZURE_OPENAI_ENDPOINT mapping: {preview}");
    assert!(preview.contains("-e AZURE_API_BASE=https://example.invalid/base"), "missing AZURE_API_BASE mapping: {preview}");
    assert!(preview.contains("-e AZURE_OPENAI_API_VERSION=2024-10-01"), "missing AZURE_OPENAI_API_VERSION mapping: {preview}");
    assert!(preview.contains("-e AZURE_API_VERSION=2024-10-01"), "missing AZURE_API_VERSION mapping: {preview}");

    // Restore env
    if let Some(v) = old_key { std::env::set_var("AIFO_API_KEY", v); } else { std::env::remove_var("AIFO_API_KEY"); }
    if let Some(v) = old_base { std::env::set_var("AIFO_API_BASE", v); } else { std::env::remove_var("AIFO_API_BASE"); }
    if let Some(v) = old_ver { std::env::set_var("AIFO_API_VERSION", v); } else { std::env::remove_var("AIFO_API_VERSION"); }
}
