...
fn test_cli_fork_merge_octopus_autoclean_color_success() {
    ...
    let clones = aifo_coder::fork_clone_and_checkout_panes(repo, sid, 2, &cur_branch, &base_label, false).expect("clone");
    for (idx, (pane_dir, _)) in clones.iter().enumerate() {
        ...
        assert!(Command::new("git").args(["commit","-m","ok"]).current_dir(pane_dir).status().unwrap().success());
    }

    // run merge with autoclean and forced color
    let bin = env!("CARGO_BIN_EXE_aifo-coder");
    let out_cli = Command::new(bin)
        .args(["--color","always","fork","merge","--session",sid,"--strategy","octopus","--autoclean"])
        .current_dir(repo)
        .output()
        .expect("run aifo-coder fork merge");
    assert!(out_cli.status.success(), "merge failed: stderr={}", String::from_utf8_lossy(&out_cli.stderr));

    let stderr = String::from_utf8_lossy(&out_cli.stderr);
    assert!(stderr.contains("\x1b["), "expected ANSI color in stderr, got:\n{}", stderr);
    assert!(stderr.contains("octopus merge succeeded"), "expected success message in stderr, got:\n{}", stderr);
}
...
fn test_cli_fork_merge_octopus_color_failure() {
    ...
    // pane 1
    {
        ...
        let _ = Command::new("git").args(["config","user.email","aifo@example.com"]).current_dir(pane_dir).status();
        assert!(Command::new("git").args(["add","-A"]).current_dir(pane_dir).status().unwrap().success());
        assert!(Command::new("git").args(["commit","-m","p1"]).current_dir(pane_dir).status().unwrap().success());
    }
    // pane 2
    {
        ...
        let _ = Command::new("git").args(["config","user.email","aifo@example.com"]).current_dir(pane_dir).status();
        assert!(Command::new("git").args(["add","-A"]).current_dir(pane_dir).status().unwrap().success());
        assert!(Command::new("git").args(["commit","-m","p2"]).current_dir(pane_dir).status().unwrap().success());
    }

    // run merge with forced color; expect failure
    let bin = env!("CARGO_BIN_EXE_aifo-coder");
    let out_cli = Command::new(bin)
        .args(["--color","always","fork","merge","--session",sid,"--strategy","octopus"])
        .current_dir(repo)
        .output()
        .expect("run aifo-coder fork merge");
    ...
    let stderr = String::from_utf8_lossy(&out_cli.stderr);
    assert!(stderr.contains("\x1b["), "expected ANSI color in stderr, got:\n{}", stderr);
    assert!(stderr.to_ascii_lowercase().contains("failed"), "expected failure message in stderr, got:\n{}", stderr);
}
