...
fn test_apparmor_env_fallback_to_docker_default_if_available() {
    ...
    std::env::set_var("AIFO_CODER_APPARMOR_PROFILE", "surely-not-present-profile-xyz");
    let prof = aifo_coder::desired_apparmor_profile_quiet();
    // Restore env
    if let Some(v) = old { std::env::set_var("AIFO_CODER_APPARMOR_PROFILE", v); } else { std::env::remove_var("AIFO_CODER_APPARMOR_PROFILE"); }

    if let Some(p) = prof {
        assert_eq!(p, "docker-default", "expected fallback to docker-default, got {}", p);
    } else {
