Title: OpenTelemetry support for aifo-coder (v7)
Status: Proposed
...
v7 consolidates prior iterations into a precise, production-ready specification with rigorous safeguards:
...
- explicit W3C TraceContext propagator setup (no baggage by default)
- error status propagation on spans (using OpenTelemetry span extensions)
- metrics instruments with clear units, cumulative temporality, and low cardinality
- idempotent initialization and safe shutdown (no panics, no exit code changes)
- cohesive phased delivery and testing guidance (including golden stdout test)
...
- Support local development with “stdout” exporters routed to stderr (or safe file sink); support production export via OTLP/gRPC.
...
- AIFO_CODER_OTEL_METRICS_FILE
  - Optional override path for the development metrics file sink. Default sink path when stdout-metrics cannot target stderr:
...
- Propagator:
  ...
  - Note: baggage is not installed by default to avoid accidental PII propagation.
...
- Metrics exporter selection (only when AIFO_CODER_OTEL_METRICS=1):
  ...
  - Otherwise (dev): configure stdout metrics exporter writing to stderr if supported; if not supported, sink to:
    - ${AIFO_CODER_OTEL_METRICS_FILE} if set and non-empty, else
    - ${XDG_RUNTIME_DIR:-/tmp}/aifo-coder.otel.metrics.jsonl (JSONL).
...
- Histograms (unit "s" with cumulative temporality):
...
- With "otel" enabled and AIFO_CODER_OTEL=1:
  - Golden stdout test:
    - Run a short command (e.g., --help) with telemetry enabled (stdout-to-stderr exporters), assert stdout is byte-for-byte identical to baseline.
...
Phase 5: Propagation to aifo-shim (optional, v7.x)
