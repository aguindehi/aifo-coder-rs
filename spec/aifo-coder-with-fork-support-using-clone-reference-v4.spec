AIFO-Coder Fork Support (Clone+Reference) v4 Specification
...
- Supported host platforms:
  ...
  - Windows:
    ...
    - Native Windows: Windows Terminal (wt.exe) when available; otherwise PowerShell windows; Git Bash (“Git Shell” / mintty) also supported and assumed installed because Git is required
- Container runtime: Docker CLI available in PATH on all platforms.
- Git: Git CLI available in PATH on all platforms (required; on Windows, Git Bash/Git Shell is expected to be available).

Overview
This v4 specification consolidates and expands the fork orchestration via git clone --reference-if-able with optional --dissociate, adds repository-scoped locking (including Windows Git Shell), per-pane state mounts, a non-destructive snapshot strategy using a temporary index and commit-tree, cross-platform orchestrators (tmux/wt/PowerShell/Git Bash), comprehensive user-facing diagnostics and recovery guidance, fork maintenance commands (list/clean) with safety protections, and automatic pruning notices for stale sessions. Clones are created under <repo-root>/.aifo-coder/forks/<sid>/pane-1..N and persist for inspection and merging unless explicitly cleaned.

Key Improvements vs v3
- Safe snapshot of dirty workspace using a temporary index (GIT_INDEX_FILE) and git commit-tree; does not alter user index/HEAD, does not invoke hooks or require signing or identity.
- Repository-scoped locking to allow concurrent runs across different repositories (including Windows Git Shell); child panes honor skip-lock.
- Per-pane state mounts for .aider/.codex/.crush to avoid shared-state races.
- Cross-platform orchestration: tmux (Linux/macOS/WSL), Windows Terminal (wt.exe), PowerShell windows, and Git Bash fallback.
- Transparency and recovery: clearly print clone paths, branches, snapshot SHA, per-pane env, and recovery tips; keep clones on orchestration failure by default.
- Maintenance: fork list/clean with JSON output, dry-run, safety checks, and pruning notices; optional auto-clean via env.
- Safety: fork clean refuses to delete panes with uncommitted changes or unique commits unless --force or selectively with --keep-dirty.
- Signing policy: clones inherit repo-level settings; launcher does not persist changes; per-process envs can disable signing for Aider without persisting.

CLI Additions (src/main.rs)
- Extend Cli with (fork mode):
  ...
  - #[arg(long)] fork_keep_on_failure: bool            // keep created clones on orchestration failure (default: keep)
- Maintenance subcommands:
  - aifo-coder fork list [--json] [--all-repos]
  - aifo-coder fork clean [--session <sid> | --older-than <days> | --all] [--dry-run] [--yes] [--force | --keep-dirty]
- Behavior:
...
- Metadata:
  - Write <repo-root>/.aifo-coder/forks/<sid>/.meta.json with:
    { "created_at": epoch_secs, "base_label": "...", "base_ref_or_sha": "...", "snapshot_sha": "..." (optional), "base_commit_sha": "...", "panes": N, "pane_dirs": ["..."], "branches": ["..."], "layout": "..." }
...
- fork clean:
  - Deletes sessions and clone directories with safety protections (see below).
  - Modes:
    ...
    - Safety override/selection:
      - --force: override safety and delete protected panes/sessions anyway
      - --keep-dirty: delete only clean panes; keep panes that are dirty/ahead/base-unknown
  - Safety checks (default protective behavior):
    - Dirty panes: git -C <pane-dir> status --porcelain=v1 -uall non-empty => protected
    - Ahead panes: unique commits beyond fork base; compute base_commit_sha (snapshot_sha or resolved base_ref_or_sha). If git -C <pane-dir> rev-list --count <base_commit_sha>..HEAD > 0 => protected
    - Base unknown: if base_commit_sha cannot be resolved, treat as protected and report
    - Submodules: best-effort git -C <pane-dir> submodule status --recursive; if output indicates changes (“+”, “-”, “U”), consider dirty (non-fatal)
  - Behavior:
    - Default: if any targeted session has protected panes, refuse deletion; print a summary with reasons and hints; exit non-zero
    - --keep-dirty: delete only clean panes; update .meta.json; remove session dir only if all panes were deleted
    - --force: delete everything regardless; print a strong warning
...
- On normal (non-fork) runs, the launcher scans <repo-root>/.aifo-coder/forks for old sessions:
  ...
  - Protected sessions (dirty/ahead) are never deleted automatically by default.
- Optional automatic pruning:
  - If AIFO_CODER_FORK_AUTOCLEAN=1 is set, apply --older-than threshold automatically at startup for sessions where all panes are clean; print summary of deletions and survivors.
  - Future optional override: AIFO_CODER_FORK_AUTOCLEAN_FORCE=1 to force deletions (not recommended).
...
- Unit tests:
  ...
  - fork clean safety planning:
    - Dirty pane detection via status --porcelain
    - Ahead count via rev-list base..HEAD
    - Base unknown behavior
    - --keep-dirty and --force planning output
...
- Maintenance:
  ...
  - fork clean safety behavior enforced by default; --keep-dirty and --force behave as specified.
  - Autoclean respects safety by default; prints pruning suggestions.
...
- Phase 6: Maintenance, safety and pruning
  - Implement fork list and fork clean:
    - Safety planning (dirty/ahead/base-unknown) with default refusal to delete protected panes.
    - --keep-dirty deletes only clean panes; --force overrides safety.
    - JSON output; dry-run; safety prompts; .meta.json updates on partial deletions.
  - Startup stale-session notice; optional auto-clean controlled by env; honor safety by default.
  - Tests: listing/cleaning logic; safety scenarios; threshold handling; messaging.

- Phase 7: Polish and docs
  - Diagnostics, large-N warnings, error messages, merging guidance in help output.
  - Update user-facing docs and help:
    - Update README.md (fork mode overview, examples, safety and recovery guidance).
    - Update INSTALL.md (requirements for tmux/wt.exe/Git Bash).
    - Update Makefile help target lines to mention fork list/clean and safety flags.
    - Update aifo-coder --help to include fork subcommands and flags.
    - Update man page man/aifo-coder.1 with a dedicated FORK MODE section, commands, safety, and merging guidance.
  - Update repository spec references to point to this v4 document.
...
- src/main.rs:
  ...
  - Add fork subcommands: fork list, fork clean (args as above, including --force and --keep-dirty).
  ...
  - Maintenance:
    ...
    - fork_clean(repo_root, opts) -> ExitCode
      - Plan protection status per pane (dirty/ahead/base-unknown); honor --keep-dirty/--force.
      - Update .meta.json after partial deletions.
...
- src/lib.rs:
  ...
  - Locking helpers:
    ...
    - candidate_lock_paths() updated to prefer repo-scoped paths when in a repo; legacy list when not.
...
- Safety checks performance on very large histories: use rev-list --count (fast) and bail fast on errors by treating panes as protected.
