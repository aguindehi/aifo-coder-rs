Title: OpenTelemetry support for aifo-coder (v3)
Status: Proposed
...
This document refines v2 with explicit safeguards to preserve CLI UX, robust exporter/runtime handling, privacy-first defaults, and a complete phased plan including metrics. Telemetry remains fully optional (compile- and runtime-gated). When enabled, it must not pollute stdout, must never alter exit codes, and must degrade gracefully on failures.

1) Goals

- Add optional OpenTelemetry tracing and metrics to aifo-coder without changing default UX or tests.
...
- Support local development via stdout exporters (but not to stdout; see below) and production via OTLP.
...
- No new CLI flags for telemetry in v3; runtime via environment only.
- No changes to user-facing stdout/stderr by default; telemetry must not write to stdout.
- No external span linking to third-party services in v3 (only in-process and shim/proxy boundary).

3) Build and feature gating (Cargo)

Features (off by default):
- otel: enables tracing and OpenTelemetry (stdout exporters available for traces/metrics).
...
PII handling:
- AIFO_CODER_OTEL_PII
  - Default "0": redact PII (paths/args). When "1", include raw cwd and args in spans (unsafe debugging mode).
...
Behavior:
- Enablement: If neither AIFO_CODER_OTEL=1 nor a non-empty OTEL_EXPORTER_OTLP_ENDPOINT is present, return None immediately.
- Resource attributes (conservative):
  ...
  - host.name, os.type, process.pid (best-effort; numeric for pid)
- TextMap propagator:
...
- Traces exporter selection:
  - If feature "otel-otlp" is enabled and OTEL_EXPORTER_OTLP_ENDPOINT is set (non-empty):
    - Use opentelemetry-otlp pipeline (gRPC tonic) with a batch span processor.
...
- Idempotence:
  - Protect initialization with a process-wide OnceLock/OnceCell to prevent double-initialization. Subsequent calls return None without side effects.
- TelemetryGuard:
  - Holds any shutdown handles (provider/runtime).
  - Implements Drop to:
    - call opentelemetry::global::shutdown_tracer_provider()
    - force-flush metrics provider (if available)
...
General:
- Use #[cfg_attr(feature = "otel", tracing::instrument(...))] on functions so the attributes compile away when otel is off.
...
- toolchain_start_session(kinds, overrides, no_cache, verbose)
  ...
  - Add events per sidecar started; errors set span status to error.
...
- toolexec_start_proxy(session_id, verbose)
  ...
  - Inside request loop (HTTP and Unix):
    ...
    - After execution, record exit_code (int) and dur_ms (u128). On timeout/error, set span status to error.
...
- preferred_registry_prefix() / preferred_registry_prefix_quiet()
  ...
  - For the active probe path, record method and result; avoid raw network details.
...
- On failures (docker non-zero, proxy timeouts, lock contention), set span status to error with concise message.
...
- Else (local dev): configure stdout metrics exporter (very low volume), sink to stderr/file (not stdout).
...
- Cardinality and PII:
  - Only low-cardinality labels (agent, kind, tool, result, source). No paths, usernames, hashes, or secrets.
...
- Never record secrets or token/header values (including OTEL_EXPORTER_OTLP_HEADERS).
...
- OTLP uses a batch processor with bounded queues and timeouts to avoid stalling the CLI.
- Keep spans concise; use skip() in #[instrument] for large args.
- Optionally install a conservative EnvFilter (warn) only when fmt is opted in; otherwise rely on layer defaults.
...
- Double initialization attempts:
  - Protected by OnceLock/OnceCell; subsequent calls return None without side effects.
...
- Add a CI smoke run that diff-checks stdout output with and without telemetry enabled to ensure no differences (since exporters write to stderr).
...
- Respect OTEL_TRACES_SAMPLER / OTEL_TRACES_SAMPLER_ARG and OTEL_EXPORTER_OTLP_TIMEOUT.
...
- Add counters and histograms per section 7 with low-cardinality labels; set proper units.
- Ensure force-flush on guard drop.

Phase 5: Propagation to aifo-shim (optional, v3.x)
- Inject/propagate W3C traceparent across the shim/proxy boundary (HTTP/Unix).
- Extract context on proxy side and create child spans for request execution.
...
- Implement telemetry_init() and TelemetryGuard:
  ...
  - try_init(), idempotence via OnceLock/OnceCell, concise warnings on conflict
...
pub fn telemetry_init() -> Option<TelemetryGuard> {
    ...
    // host.name/os.type may be added best-effort
    ...
    let (tracer, maybe_rt) = if use_otlp {
        ...
        // Configure timeouts from env or default 5s
