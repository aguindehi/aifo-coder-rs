...
Owner: aifo-coder  
Date: 2025-12-18  
Scope: make the embedded shim (`aifo-shim`) the single source of truth for
routing decisions (local vs proxied), eliminate per-agent PATH special-casing
in the launcher, and converge the shim implementations to one auditable design.

This spec supersedes v1 and addresses gaps discovered during implementation:
- Multiple shim implementations exist in-tree (POSIX script shim, Rust proxy shim,
  Rust smart shim).
- Launcher PATH logic still varies by agent as a safety belt.
- The intended end-state is a unified, deterministic shim that always sits first
  in PATH and routes correctly based on strict policy.

## Background / problem statement

The agent images place `/opt/aifo/bin` early in PATH and symlink common tools
(node/python/...) to `aifo-shim` so tool execution can be proxied to toolchain
sidecars and logged/policy-controlled.

However, runtime-based agents (Node/Python) may spawn their own runtime for
internal entrypoints located outside `/workspace`. If these invocations get
proxied into sidecars, agents can fail because their internal packages are in
the agent image, not in the sidecar.

v1 introduced “smart shims”: the shim decides when to run locally vs proxy.
During implementation, additional complexity emerged:
- The repo contains:
  - a POSIX shim generator (`toolchain_write_shims`) that writes `/opt/aifo/bin/aifo-shim`
    as a shell script that proxies.
...
v2 resolves these inconsistencies by converging to a single shim implementation
and making PATH consistently shim-first.
...
2. **Uniform launcher PATH**: the launcher always uses the same PATH shape and
   always places `/opt/aifo/bin` first; no per-agent PATH reordering.
3. **Deterministic, conservative bypass rules**:
   - Smart behavior is opt-in via env and tool toggles (as in v1).
   - Bypass decisions are based only on tool name, argv, cwd, and fixed prefix
     checks (no user-controlled regex policy in v2).
4. **Backwards compatibility**:
   - Existing toolchain runs continue to work (proxy protocol, signals, exit codes,
     `/notify` behavior).
...
- **Local execution**: run the real runtime inside the agent container using an
  absolute runtime path (e.g. `/usr/bin/python3`, `/usr/local/bin/node`).
- **Proxied execution**: execute tool through the toolexec proxy (HTTP/UDS) into
  toolchain sidecars, preserving existing behavior (signals, streaming, exit code).
...
- Rust proxy shim (`src/bin/aifo-shim.rs`):
  - full proxy client; supports native HTTP/UDS + curl fallback, signals, `/notify`,
    trailer parsing, disconnect handling, and logging.
- Rust smart shim (`src/aifo-shim/main.rs`):
  - local-vs-proxy decisions for node/python, plus a proxy delegation path.

This violates “single source of truth” and makes it unclear which behavior
actually executes when `/opt/aifo/bin` is first in PATH.
...
- **`/opt/aifo/bin/aifo-shim` is a Rust binary** implementing:
  ...
  - proxied execution, including `/notify` and `/signal`, preserving the existing
    protocol/UX semantics.
...
- The generated POSIX `aifo-shim` script must not be installed as `aifo-shim` in v2.
  - If a POSIX wrapper is retained, it must be a thin `exec` trampoline into the
    Rust binary and must not implement routing/proxy itself.
- Tool wrappers must not create recursion (no `exec aifo-shim` via PATH):
  prefer `exec "$(dirname "$0")/aifo-shim" "$@"`.
...
- Parse argv to identify the “main program” (best-effort):
  ...
  - Skip known node options that consume the next value (e.g. `-r/--require`,
    `--loader`, `--import`, etc.).
...
- Existing disconnect handling semantics (including shell-kill behavior) to avoid
  lingering shells after Ctrl-C or stream disconnects.

v2 decision: **fuse smart-routing into the existing Rust proxy shim implementation**
so there is a single binary:
...
- Confirm that POSIX shim generation will no longer produce a conflicting
  `aifo-shim` implementation.
...
- Ensure the runtime shim directory installs `aifo-shim` as the Rust binary and does
  not install a proxy-implementing POSIX `aifo-shim` script.
...
2) **Launcher wiring** (unit tests):
   - For node agents: env includes `AIFO_SHIM_SMART=1`, `AIFO_SHIM_SMART_NODE=1`,
     and `AIFO_AGENT_NAME=<agent>`.
   - For python agents: env includes `AIFO_SHIM_SMART=1`, `AIFO_SHIM_SMART_PYTHON=1`,
     and `AIFO_AGENT_NAME=<agent>`.
...
3) **Integration smoke** (optional/ignored; requires Docker):
   ...
   - Verify that `node` invoked on an agent-internal script outside `/workspace`
     does not hit the proxy (can be inferred by absence of proxy log lines).
...
1. With toolchain enabled and `/opt/aifo/bin` first in PATH, node-based agents
   (`letta/codex/crush/opencode`) can start and function without proxying their
   internal runtime entrypoints outside `/workspace`.
2. Python-based agents (`aider/openhands`) can start and function without proxying
   internal module/script execution.
3. Tool invocations under `/workspace` continue to proxy as before.
4. No PATH special-casing remains in launcher code.
5. `/opt/aifo/bin/aifo-shim` is the only shim implementation required at runtime
   (no `aifo-shim-proxy`, no proxy-implementing POSIX script).
6. `make check` passes.
