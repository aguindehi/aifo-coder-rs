Title: OpenTelemetry support for aifo-coder (v5)
Status: Proposed
...
v5 consolidates prior iterations with rigor around stderr-only development exporters (traces and metrics), strict fmt-layer opt-in, reliable flush on short-lived CLI runs, precise enablement and fallback rules, explicit propagator setup, error status propagation, metrics units/temporality, and clear phased delivery and testing. Telemetry is fully optional (compile- and runtime-gated) and must not alter CLI stdout, existing stderr messages by default, or exit codes.

1) Goals

- Add optional, opt-in OpenTelemetry tracing and metrics without changing default CLI UX or test behavior.
...
- Include metrics (counters and histograms) behind a runtime gate; disabled by default.
- Support local development with “stdout exporters” routed to stderr; support production export via OTLP/gRPC.
- Enforce privacy defaults (salted hashes and counts) to avoid PII leakage by default.
...
- No changes to user-facing stdout/stderr by default; telemetry never writes to stdout.
- No third-party service span linking in v5 (only in-process and aifo-shim boundary later).

3) Build and feature gating (Cargo)

Features (off by default):
- otel: enables tracing and OpenTelemetry (stdout exporters available for traces and metrics).
- otel-otlp: extends otel with OTLP exporter and a private Tokio runtime dependency.

Dependencies in Cargo.toml (all optional = true; versions pinned to avoid SDK churn):
...
- once_cell = "1"  # process-wide idempotent initialization
...
Default build has neither feature enabled; telemetry code compiles out.

4) Runtime configuration (environment)

Enablement (explicit and stable rules):
- AIFO_CODER_OTEL=1
...
- AIFO_CODER_TRACING_FMT=1
  - Strict opt-in to install fmt layer so tracing events can appear on stderr. Without this flag, fmt is NOT installed.
- RUST_LOG
  - Used only as an EnvFilter when fmt is installed. If fmt is not installed, RUST_LOG alone has no effect on user-visible logs.
...
- Traces exporter selection:
  - If feature "otel-otlp" is enabled and OTEL_EXPORTER_OTLP_ENDPOINT is set (non-empty):
    ...
    - Respect OTEL_EXPORTER_OTLP_TIMEOUT and OTEL_TRACES_SAMPLER variables.
  - Else:
    - Use opentelemetry-stdout exporter for traces.
    - Use a simple span processor to ensure predictable flush for short-lived CLI.
    - MUST: configure the stdout exporter to write to stderr (not stdout), or equivalent writer sink, to avoid polluting CLI stdout.
- Metrics exporter selection (only when AIFO_CODER_OTEL_METRICS=1):
  ...
  - Otherwise: configure stdout metrics exporter for local dev (low volume), writing to stderr or a local file sink (never stdout).
    - If the stdout metrics exporter cannot target stderr, sink to ${XDG_RUNTIME_DIR:-/tmp}/aifo-coder.otel.metrics.jsonl (JSONL).
- Subscriber installation:
  - Always install the tracing_opentelemetry layer bound to the tracer.
  - Install fmt layer only when AIFO_CODER_TRACING_FMT=1. When installed, honor RUST_LOG as EnvFilter; default filter is "warn" for conservative output.
  - When fmt is not installed, do not produce any additional stderr/stdout logs beyond current behavior.
...
- TelemetryGuard:
  ...
  - Implements Drop to:
    ...
    - force-flush the meter provider (if available) with a short timeout to avoid losing metrics in short CLI runs
...
- Levels:
  ...
  - Default fmt not installed, so user-facing stderr is unchanged unless explicitly opted in.

Hash redaction helper:
- Implement FNV-1a 64-bit with a per-process salt (derive from pid and start_nanos).
- When AIFO_CODER_OTEL_PII != "1", record only counts and salted hashes (args_count, cwd_hash).
- When AIFO_CODER_OTEL_PII = "1", include raw cwd and args strings with caution; still avoid file contents, secrets, and env values.
...
- On failures (docker non-zero, proxy timeouts, lock contention), set span status to error with a concise message via tracing_opentelemetry::OpenTelemetrySpanExt.

7) Metrics plan (opt-in via AIFO_CODER_OTEL_METRICS=1)

Exporters/readers:
- With otel-otlp + endpoint: configure OTLP metrics exporter with a PeriodicReader (interval ~1–2s; CLI-friendly).
- Else (dev): configure stdout metrics exporter and route to stderr; if unsupported, write to ${XDG_RUNTIME_DIR:-/tmp}/aifo-coder.otel.metrics.jsonl. Never stdout.

Force flush:
- On TelemetryGuard::drop, force-flush the meter provider (short timeout) to avoid losing metrics in short runs.
...
Temporality:
- Use cumulative temporality unless the backend requires delta; document defaults and any changes.

Cardinality and PII:
- Only low-cardinality labels (agent, kind, tool, result, source). No paths, usernames, hashes, or secrets in metrics.

Instrumentation points:
- Increment counters at the correct points (e.g., around docker run/exec, proxy requests).
...
Stdout-dev exporters (stderr sinks):
...
- With metrics enabled at runtime:
  - AIFO_CODER_OTEL=1 AIFO_CODER_OTEL_METRICS=1 cargo run --features otel -- --help (stdout metrics sink to stderr or file)
  - With otel-otlp exporter and collector, verify metrics reach the collector (manual).

Golden stdout test (recommended):
- Add a CI smoke run that diff-checks stdout output with and without telemetry enabled to ensure no differences (since exporters write to stderr or file). This protects stdout invariants.
...
Phase 5: Propagation to aifo-shim (optional, v5.x)
...
- Add cfg(feature="otel") imports:
  - tracing::{instrument, info, debug, warn, error}
...
- Implement stdout exporters with a stderr writer sink for both traces and metrics; if metrics writer unsupported, write to ${XDG_RUNTIME_DIR:-/tmp}/aifo-coder.otel.metrics.jsonl.
...
- On failures, set span status error with concise messages.
...
- Units:
  - Counters: unit "1"
  - Durations: unit "s" (preferred). If "ms" is used, be consistent across all instruments.
...
pub fn telemetry_init() -> Option<TelemetryGuard> {
    ...
    let (tracer, maybe_rt) = if use_otlp {
        ...
        // e.g., opentelemetry_stdout::SpanExporter::builder().with_writer(std::io::stderr()).build()
        (/* tracer */, None)
    };
    ...
    let _maybe_meter_provider = if std::env::var("AIFO_CODER_OTEL_METRICS").ok().as_deref() == Some("1") {
        if use_otlp {
            ...
            // stdout metrics to stderr if supported; else JSONL file in ${XDG_RUNTIME_DIR:-/tmp}
            Some(/* meter provider */)
        }
    } else {
        None
...
- Stay consistent with once_cell::sync::OnceCell for idempotence (aligns with crate usage).
...
This v5 specification ensures zero stdout interference (stderr-only dev exporters or file sink), strict fmt opt-in, robust OTLP handling with a private runtime, privacy-preserving spans, optional metrics with force-flush, and a safe phased rollout with clear testing guidance. It defines precise enablement rules, resilient initialization and shutdown behavior, and targeted instrumentation with low overhead and low cardinality.
