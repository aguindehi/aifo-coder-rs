...
- Image selection and fallback bootstrap:
  - Default image is aifo-coder-toolchain-rust:<version|latest>.
...
- Optional mounts/env:
  ...
  - sccache: If AIFO_RUST_SCCACHE=1,
    ...
    - If sccache binary is absent:
      - On aifo-coder-toolchain-rust images: SHOULD be present; otherwise, install via package manager/cargo at runtime or fail with clear message (policy‑based).
