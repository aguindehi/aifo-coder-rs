Title: OpenTelemetry support for aifo-coder (v6)
Status: Proposed
...
v6 consolidates prior iterations with rigorous safeguards:
- stderr-only development exporters (both traces and metrics), never stdout
- strict fmt-layer opt-in (no surprise stderr logs)
- reliable flush for short-lived CLI runs (traces and metrics)
- explicit and stable enablement/fallback rules
- explicit W3C TraceContext propagator setup
- error status propagation on spans
- metrics instruments with clear units/temporality and low cardinality
- idempotent initialization and safe shutdown
- phased delivery and testing guidance

Telemetry remains fully optional (compile- and runtime-gated). When enabled, it must not alter CLI stdout, default stderr messages, or exit codes.
...
- Support local development with stdout exporters routed to stderr (or file sink); support production export via OTLP/gRPC.
- Enforce privacy-by-default (salted hashes and counts), with an explicit unsafe debugging override.
...
- Traces exporter selection:
  - If feature "otel-otlp" is enabled AND OTEL_EXPORTER_OTLP_ENDPOINT is non-empty:
    - Use opentelemetry-otlp (tonic gRPC) with a batch span processor tuned for CLI:
      - Defaults (CLI-friendly) if OTEL_BSP_* unset:
        - schedule delay ≈ 2s
        - max queue size ≈ 2048
        - export timeout ≈ 5s
  ...
  - Else:
    ...
    - MUST: configure the stdout exporter to write to stderr (not stdout), or equivalent writer sink.
- Metrics exporter selection (only when AIFO_CODER_OTEL_METRICS=1):
  ...
  - Otherwise: configure stdout metrics exporter for local dev, writing to stderr if supported; if not, sink to ${XDG_RUNTIME_DIR:-/tmp}/aifo-coder.otel.metrics.jsonl (JSONL). Never stdout.
...
- With "otel" enabled and AIFO_CODER_OTEL=1:
  ...
  - Verify no stdout changes; telemetry prints to stderr only when fmt is enabled or stdout exporters are active (to stderr sink or JSONL file for metrics if needed).
...
- Add a CI smoke run that diff-checks stdout with and without telemetry enabled to ensure no differences (since exporters write to stderr or file). This protects stdout invariants.
...
Short-run flush test:
- Validate that a very short CLI run (e.g., --help) flushes traces and, when enabled, metrics (dev stdout-to-stderr or file sink). No hangs.
...
- Ensure force-flush on guard drop (metrics provider).

Phase 5: Propagation to aifo-shim (optional, v6.x)
...
pub fn telemetry_init() -> Option<TelemetryGuard> {
    ...
    let (tracer, maybe_rt) = if use_otlp {
        ...
        // Configure timeouts from env or default 5s and batch params from OTEL_BSP_* (delay ~2s, queue ~2048)
