...
fn build_container_sh_cmd(path_value: &str, agent_joined: &str) -> io::Result<String> {
    ...
    sh.extend([
        ...
        r#"if [ "$need_user" = "1" ]; then"#.to_string(),
        r#"  mkdir -p "$HOME/.nss_wrapper""#.to_string(),
        r#"  PASSWD_FILE="$HOME/.nss_wrapper/passwd"; GROUP_FILE="$HOME/.nss_wrapper/group""#.to_string(),
        r#"  echo "coder:x:${uid}:${gid}:,,,:$HOME:/bin/sh" > "$PASSWD_FILE""#.to_string(),
        r#"  echo "coder:x:${gid}:" > "$GROUP_FILE""#.to_string(),
        r#"  for so in /usr/lib/*/libnss_wrapper.so /usr/lib/*/libnss_wrapper.so.* /usr/lib/libnss_wrapper.so /lib/*/libnss_wrapper.so /lib/*/libnss_wrapper.so.*; do if [ -f "$so" ]; then export LD_PRELOAD="${LD_PRELOAD:+$LD_PRELOAD:}$so"; break; fi; done"#.to_string(),
        r#"  export NSS_WRAPPER_PASSWD="$PASSWD_FILE" NSS_WRAPPER_GROUP="$GROUP_FILE" USER="coder" LOGNAME="coder""#.to_string(),
        r#"fi"#.to_string(),
