        ...
        "node" => {
            ...
            let bootstrap = ShellScript::new()
                .extend([
                    ...
                    r#"if [ ! -d "$d" ] || [ -z "$(ls -A "$d" 2>/dev/null || true)" ]; then"#.to_string(),
                    r#"  mkdir -p "$d""#.to_string(),
                    r#"  if [ -f "$s" ]; then"#.to_string(),
                    r#"    if command -v pnpm >/dev/null 2>&1; then"#.to_string(),
                    r#"      echo "aifo-coder: node sidecar: bootstrapping node_modules via pnpm install --frozen-lockfile" >&2"#.to_string(),
                    r#"      PNPM_STORE_PATH="${PNPM_STORE_PATH:-/workspace/.pnpm-store}" pnpm install --frozen-lockfile || true"#.to_string(),
                    r#"    else"#.to_string(),
                    r#"      echo "aifo-coder: warning: pnpm not found in node toolchain image; skipping automatic install" >&2"#.to_string(),
                    r#"    fi"#.to_string(),
                    r#"  fi"#.to_string(),
                    r#"fi"#.to_string(),
                    format!(
...
fn node_overlay_state_and_guard(
    ...
    let script = ShellScript::new()
        .extend([
            ...
            r#"if [ -f "$s" ]; then"#.to_string(),
            r#"  if [ "$(stat -c '%d:%i' "$wd" 2>/dev/null || echo '?')" = "$(stat -c '%d:%i' "$nd" 2>/dev/null || echo '!')" ]; then"#.to_string(),
            r#"    echo "error:overlay-device-mismatch"; exit 0;"#.to_string(),
            r#"  fi"#.to_string(),
            r#"fi"#.to_string(),
...
fn ensure_node_overlay_and_install(
    ...
    let script = ShellScript::new()
        .extend([
            ...
            r#"mkdir -p "$d""#.to_string(),
            r#"if [ -f "$lock" ] && command -v sha256sum >/dev/null 2>&1; then"#.to_string(),
            r#"  new_hash="$(sha256sum "$lock" 2>/dev/null | awk '{print $1}')""#.to_string(),
            r#"  old_hash="""#.to_string(),
            r#"  if [ -f "$hash_file" ]; then old_hash="$(cat "$hash_file" 2>/dev/null || echo '')"; fi"#.to_string(),
            r#"  if [ "$new_hash" != "$old_hash" ] && command -v pnpm >/dev/null 2>&1; then"#.to_string(),
            r#"    echo "aifo-coder: node sidecar: pnpm-lock.yaml changed; running pnpm install --frozen-lockfile" >&2"#.to_string(),
            r#"    PNPM_STORE_PATH="${PNPM_STORE_PATH:-/workspace/.pnpm-store}" pnpm install --frozen-lockfile || true"#.to_string(),
            r#"    printf '%s\n' "$new_hash" >"$hash_file" 2>/dev/null || true"#.to_string(),
            r#"  fi"#.to_string(),
            r#"elif [ -f "$lock" ] && command -v pnpm >/dev/null 2>&1; then"#.to_string(),
            r#"  echo "aifo-coder: node sidecar: ensuring node_modules via pnpm install --frozen-lockfile" >&2"#.to_string(),
            r#"  PNPM_STORE_PATH="${PNPM_STORE_PATH:-/workspace/.pnpm-store}" pnpm install --frozen-lockfile || true"#.to_string(),
            r#"fi"#.to_string(),
